...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagswitch0)
    {
        info->event[info->num] = EVENT_switch0;
        info->num++;
        dataPoints->valueswitch0 = gizStandardDecompressionValue(switch0_BYTEOFFSET,switch0_BITOFFSET,switch0_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueswitch0 != cur->valueswitch0)
    {
        GIZWITS_LOG("valueswitch0 Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(switch0_BYTEOFFSET,switch0_BITOFFSET,switch0_LEN,(uint8_t *)devStatusPtr,dataPoints->valueswitch0);
